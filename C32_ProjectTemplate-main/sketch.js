...
for(var i = 0 ; i < playerArrows.length ; i++){
var colisaoJC = Matter.SAT.collides(playerArrows[i].body,computerArcher.body);
if(colisaoJC.collided){
  console.log("COLIDIU");
}
}
}                                         
...
function handleComputerArrowCollision(){
  ...
  for(var b = 0 ; b < computerArrows.length ; b++){
  var colisaoCJ = Matter.SAT.collides(computerArrows[b].body,playerArcher.body);

  if(colisaoCJ.collided){
    console.log("COLIDIU");
  }
 }
}